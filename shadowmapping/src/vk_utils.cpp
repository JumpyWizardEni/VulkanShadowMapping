...
VkInstance vk_utils::CreateInstance(bool a_enableValidationLayers, std::vector<const char *> &a_enabledLayers,
                                    std::vector<const char *> a_extentions) {
...
void vk_utils::InitDebugReportCallback(VkInstance a_instance, DebugReportCallbackFuncType a_callback,
    ...
    //
    auto vkCreateDebugReportCallbackEXT = (PFN_vkCreateDebugReportCallbackEXT) vkGetInstanceProcAddr(a_instance,
...
VkDevice vk_utils::CreateLogicalDevice(uint32_t queueFamilyIndex, VkPhysicalDevice physicalDevice,
                                       const std::vector<const char *> &a_enabledLayers,
                                       std::vector<const char *> a_extentions) {
    ...
    deviceCreateInfo.enabledLayerCount = uint32_t(
            a_enabledLayers.size());  // need to specify validation layers here as well.
...
uint32_t
vk_utils::FindMemoryType(uint32_t memoryTypeBits, VkMemoryPropertyFlags properties, VkPhysicalDevice physicalDevice) {
...
void vk_utils::CreateSwapChain(VkPhysicalDevice a_physDevice, VkDevice a_device, VkSurfaceKHR a_surface, int a_width,
                               int a_height,
...
void vk_utils::CreateScreenFrameBuffers(VkDevice a_device, VkRenderPass a_renderPass, ScreenBufferResources *pScreen,
    ...
    for (size_t i = 0; i < pScreen->swapChainImageViews.size(); i++) {
        std::array<VkImageView, 2> attachments = {
                ...
                depthImageView[currIndex]
        };
        currIndex++;
        if (currIndex >= depthImageView.size()) currIndex = 0;
...
void vk_utils::CreateScreenFrameBuffersForDepth(VkDevice a_device, std::vector<VkFramebuffer> &framebuffer, VkRenderPass
a_renderPass,
                                                ScreenBufferResources *
                                                pScreen,
                                                VkImageView &depthImageView, size_t dim) {
    framebuffer.resize(pScreen->swapChainImageViews.size());
    for (size_t i = 0; i < pScreen->swapChainImageViews.size(); i++) {
        ...
        if (vkCreateFramebuffer(a_device, &framebufferInfo, nullptr, &framebuffer[i]) != VK_SUCCESS)
            throw std::runtime_error("failed to create framebuffer!");
    }
}
